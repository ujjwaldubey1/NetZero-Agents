    ...
    useEffect(() => {
        ...
        const logInterval = setInterval(() => {
            if (currentLog < bootSequence.length) {
                const text = bootSequence[currentLog];
                if (text) {
        ...
        }, 250); // Speed up logs to match progress
        ...
        const progressInterval = setInterval(() => {
            setProgress(prev => {
                if (prev >= 100) {
                    ...
                    setTimeout(onComplete, 800); // Wait a bit more to ensure "SYSTEM READY" is read
                    return 100;
                }
                return prev + 2;
            });
        }, 40); // Slightly slower progress (approx 2s total)
